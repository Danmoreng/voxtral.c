...
SRCS_BASE = voxtral.c voxtral_kernels.c voxtral_audio.c voxtral_encoder.c voxtral_decoder.c \
            voxtral_tokenizer.c voxtral_safetensors.c voxtral_mic_macos.c
SRCS = $(SRCS_BASE) voxtral_cuda_stub.c
OBJS = $(SRCS:.c=.o)
MAIN = main.c
TARGET = voxtral
...
.PHONY: all clean debug info help blas cuda cuda-check mps avx512 inspect test cpu
...
BLAS_CFLAGS = $(CFLAGS_BASE) -DUSE_BLAS -DUSE_OPENBLAS
...
SRCS_CUDA = $(SRCS_BASE) voxtral_cuda.c

cuda: cuda-check
	@$(MAKE) clean
	@$(MAKE) $(CUDA_CUBIN_HDR)
	@$(MAKE) $(TARGET) CFLAGS="$(CUDA_CFLAGS)" LDFLAGS="$(CUDA_LDFLAGS)" SRCS="$(SRCS_CUDA)" OBJS="$(SRCS_CUDA:.c=.o)"
	@echo ""
...
# =============================================================================
clean:
	$(CLEAN_CMD) $(OBJS) *.mps.o voxtral_metal.o main.o inspect_weights.o $(TARGET) inspect_weights test_avx512bf16_bin
	$(CLEAN_CMD) voxtral_shaders_source.h
	$(CLEAN_CMD) $(CUDA_CUBIN) $(CUDA_CUBIN_HDR)
...
voxtral_kernels.o: voxtral_kernels.c voxtral_kernels.h $(wildcard voxtral_avx512.h) voxtral_cuda.h
voxtral_cuda.o: voxtral_cuda.c voxtral_cuda.h
voxtral_cuda_stub.o: voxtral_cuda_stub.c voxtral_cuda.h
