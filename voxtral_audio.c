...
/* Global verbose flag for audio module */
...
float *vox_parse_wav_buffer(const uint8_t *data, size_t file_size, int *out_n_samples) {
    ...
    if (sample_rate != SAMPLE_RATE) {
        ...
        float *resampled = (float *)vox_cpu_malloc(new_n * sizeof(float));
        if (!resampled) {
...
float *vox_read_pcm_stdin(int *out_n_samples) {
    ...
    for (int i = 0; i < n_frames; i++) {
        int16_t val;
        memcpy(&val, &src[i], sizeof(int16_t));
...
float *vox_mel_spectrogram(const float *samples, int n_samples, int *out_frames) {
    ...
    float *padded = (float *)vox_cpu_malloc(padded_len * sizeof(float));
    ...
    float *dft_cos = (float *)vox_cpu_malloc((size_t)N_FREQ * N_FFT * sizeof(float));
    float *dft_sin = (float *)vox_cpu_malloc((size_t)N_FREQ * N_FFT * sizeof(float));
    ...
    float *mel = (float *)vox_cpu_calloc(n_frames * N_MEL, sizeof(float));
    ...
    vox_cpu_free(dft_cos);
    vox_cpu_free(dft_sin);
    vox_cpu_free(padded);
    vox_cpu_free(mel_filters);
...
static int mel_compute_available(vox_mel_ctx_t *ctx) {
    ...
    while (1) {
        ...
        if (t >= ctx->mel_cap) {
            ...
            float *tmp = (float *)vox_cpu_realloc(ctx->mel,
...
vox_mel_ctx_t *vox_mel_ctx_init(int left_pad_samples) {
    ...
    if (!ctx->mel_filters) { vox_cpu_free(ctx); return NULL; }

    /* Precompute DFT cos/sin tables */
    ctx->dft_cos = (float *)vox_cpu_malloc((size_t)N_FREQ * N_FFT * sizeof(float));
    ctx->dft_sin = (float *)vox_cpu_malloc((size_t)N_FREQ * N_FFT * sizeof(float));
    if (!ctx->dft_cos || !ctx->dft_sin) {
        vox_cpu_free(ctx->dft_cos); vox_cpu_free(ctx->dft_sin);
        vox_cpu_free(ctx->mel_filters); vox_cpu_free(ctx);
        return NULL;
    }
    for (int k = 0; k < N_FREQ; k++) {
        for (int n = 0; n < N_FFT; n++) {
    ...
    ctx->samples = (float *)vox_cpu_calloc((size_t)ctx->samples_cap, sizeof(float));
    if (!ctx->samples) {
        vox_cpu_free(ctx->dft_cos); vox_cpu_free(ctx->dft_sin);
...
int vox_mel_feed(vox_mel_ctx_t *ctx, const float *samples, int n_samples) {
    ...
    if (needed > ctx->samples_cap) {
        ...
        float *tmp = (float *)vox_cpu_realloc(ctx->samples, (size_t)new_cap * sizeof(float));
        if (!tmp) return 0;
        ctx->samples = tmp;
...
void vox_mel_free(vox_mel_ctx_t *ctx) {
    if (!ctx) return;
    vox_cpu_free(ctx->mel_filters);
    vox_cpu_free(ctx->dft_cos);
    vox_cpu_free(ctx->dft_sin);
